...
class BottleneckBlock(nn.Layer):
    def __init__(
        ...
        self.channel_atten=ECA(3)

    def forward(self, inputs):
        # import pdb;pdb.set_trace()
        y = self.conv0(inputs)
        atten = self.channel_atten(y) * y

        conv1 = self.conv1(atten)
        ...
        if self.shortcut:
            ...
            # atten_bro = self.short(atten)

        # buding
        # atten_bro = paddle.tile(atten_bro, [1, (short.shape[1] // atten_bro.shape[1]), 1, 1])

        # atten += short
        # atten_bro = paddle.add(atten_bro, short)
