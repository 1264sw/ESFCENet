...
class BottleneckBlock(nn.Layer):
    def __init__(
        ...
        # d = out_channels//2
        d = float(out_channels * (baseWidth / 64))  # d = 256 * (4/64) = 16
        d_channels = int(d*cardinality)     # d_channels = 16*32 = 512
        # print(in_channels,out_channels,d)
...
class ResNet_vd(nn.Layer):
    def __init__(self,
        ...
        self.acmix = ACmix(c2, c2)
        self.eca = ECA()

        self.stages = []
        self.out_channels = []
        if layers >= 50:
            for block in range(len(depth)):  # 循环4遍   block= 0 1 2 3
                block_list = []
                shortcut = False
                is_dcn = self.dcn_stage[block]
                for i in range(depth[block]):  # depth[block] = 3 4 6 3   depth[0] = 3    depth = [3, 4, 6, 3]   for i in range(3)
